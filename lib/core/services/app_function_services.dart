...
import '../../features/delivery/models/orders_model.dart';
...
class AppFunctions {
  static String getCurrentMonthRange() {
    ...
    return '${formatter.format(startOfMonth)} - ${formatter.format(nextMonth)}';
  }
  ...
  static StepState getStepState(RegistrationController controller, int step) {
    if (controller.completedSteps.contains(step)) return StepState.complete;
    if (controller.startedSteps.contains(step)) return StepState.editing;
  ...
  static Future<RiderDocsModel> getDocs(String? phone) async {
    ...
    // No user or phone found, return empty documents
  ...
  static Future<RiderDocsModel> getDocumentsWithMigration({
    ...
    if (migrateTo != null &&
      ...
      aadharFront = await StorageServices.getAadharFront(optionalKey);
      aadharBack = await StorageServices.getAadharBack(optionalKey);
      pan = await StorageServices.getPan(optionalKey);
      dlFront = await StorageServices.getDLFront(optionalKey);
      dlBack = await StorageServices.getDLBack(optionalKey);
      rcFront = await StorageServices.getRCFront(optionalKey);
      rcBack = await StorageServices.getRCBack(optionalKey);
    }
